...
=======
  + common.value: excluded variables which have less than 20% of being different from the most common value in monitoring data
	  - eliminates variables with little variability that are not likely to improve (or even worsen) the model fit   
	+ low.landuse:  excluded varilabies with maximum land use variables less than 10 in monitoring data
>>>>>>> e07957797efac226aaf6282d1e71983cbe91881a
...
=======
  left_join(read.csv(file.path("~", "Documents","School","PhD", "Dissertation", "TRAP R Project", "Data", "Aim 2", "Geocovariates", "201002", "dr0311_mobile_covars.csv")), by = c("location" = "native_id")) %>%
>>>>>>> e07957797efac226aaf6282d1e71983cbe91881a
...
=======
covar_names <- select(mm, log_m_to_a1:ll_a23_s05000) %>% names() 

#350 initial covariates
print(paste("there are", length(covar_names), "initial covariates"))


## --> could repeate this for other datsets (e.g., ACT geocovariates)
>>>>>>> e07957797efac226aaf6282d1e71983cbe91881a
```
...
=======
drop_variables <- unlist(drop_variables, use.names = F) %>% unique()
print(paste0(length(drop_variables), " variables were dropped"))


#variables to use for future modeling
keep_variables <- setdiff(covar_names, drop_variables)

print(paste("There are",length(keep_variables), "variables left for modeling"))
>>>>>>> e07957797efac226aaf6282d1e71983cbe91881a
...
#save final training data
mm_train <- select(mm_train0, -drop_variables)
saveRDS(mm_train, file.path("Output", "mm_cov_train_set.rda"))
>>>>>>> e07957797efac226aaf6282d1e71983cbe91881a
