...
keep_times <- stops %>%
  ...
  spread(variable, value) %>% #8150 original stops
...
# true_annual <- stops %>%
#   group_by(variable, location, route) %>%
#   summarize(value = mean(value),
#             visits = n(),
#             campaign = 1,
#             design = "full training campaign",
...
balanced_season_tow2 <- future_replicate(n = sim_n, simplify = F,
  ...
  mutate(campaign =  row_number(),
         design = "reduced temporal balance",
         version = "season TOW2") %>%
  ungroup()
...
balanced_season <- future_replicate(n = sim_n, simplify = F,
                           expr = one_sample_avg(my_list = group_split(stops_w, #variable, 
                                                                       location, season), 
                                                 my_sampling_fn = function(x) slice_sample(x, n  = 1))
                      ) %>%
  #unlist
  bind_rows() %>%
  #add simulation number & label
...
### --> ? could do 3 seasons (i.e., no sampling spring/summer/fall/winter)

## 2. shorter campaign duration 
### just samples from: summer & winter (~14 samples); spring & fall (~14 samples); individual seasons: (~7 samples)
...
season_times <- mclapply(season_times0, mc.cores = 6, FUN = function(x) {
  stops_w %>% filter(season %in% x) %>%
    mutate(version = paste(x, collapse = ", "),
           campaign =  1,
           design = "reduced temporal balance",
           )
  }
...
for(i in seq_along(visit_n)) {
  ...
  temp <- future_replicate(n = sim_n, simplify = F,
    ...
    mutate(campaign =  row_number(),
           design = "random visits",
           version = paste0(visit_n[i], " visits")
           )
...
rh_bh_df <- mclapply(rh_bh, mc.cores = 4, FUN = function(x) {
                        stops_w %>% filter(tow2 == "Weekday",
                                           hour %in% x) %>%
                          mutate(version = paste(x, collapse = ","),
                                 #version = ifelse(grepl("^7", version), "All Rush Hours", "All Business Hours"),
                                 version = ifelse(length(x) ==length(rush_hours), "Rush Hours",
...
temporal_sims <- rbind(
  ...
  mutate(spatial_temporal = "temporal")
